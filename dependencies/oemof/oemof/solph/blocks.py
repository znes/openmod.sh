...
class VariableFractionTransformer(SimpleBlock):
    ...
    **The following constraints are created:**
    ...
    def _create(self, group=None):
        ...
        self.co2_relation = Constraint(group, noruleinit=True)

        I = {n: [i for i in n.inputs.keys()][0] for n in group}
        O = {n: [o for o in n.outputs if o.sector == 'co2'] for n in group}
                    ...
                    for o in O[n]:
                        try:
                            ...
                            raise ValueError("Error in constraint creation",
                                             "source: {0}, target: {1}".format(
                                                 n.label, o.label))
                        block.co2_relation.add((n, o, t), (lhs == rhs))
        self.co2_relation_build = BuildAction(rule=_input_output_relation)
